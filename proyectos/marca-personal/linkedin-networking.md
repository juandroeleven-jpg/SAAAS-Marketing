...
```mermaid
timeline
    title LinkedIn - Avance interno
    Metodo definido : busquedas exactas Bloque A y B : criterio de senal real vs ruido
    Plantillas listas : 3 variantes de comentario genuino : mensaje de conexion : mensaje de contacto directo a empresa
    Leads reales encontrados : 13 empresas con vacante verificable : 2 excusas de contacto por vacante vencida
    Pendiente : ejecutar busquedas reales de personas : primer envio real de los 13 mensajes a empresas
...
kanban
  Listo
    l1[Metodo de busqueda, Bloques A y B]
    l2[3 variantes de comentario genuino]
    l3[Mensaje de conexion Motor 1]
    l4[13 empresas reales con mensaje de contacto Motor 2]
    l5[Tracker de 15 filas]
    l6[Criterio de decision Motor 1 vs Motor 2]
  Falta
    l7[Ejecutar busquedas reales y encontrar las 15 personas]
    l8[Comentar 2-3 posts por persona]
    l9[Enviar las 15 solicitudes de conexion]
    l10[Enviar los 13 mensajes a empresas]
    l11[Registrar resultados en el tracker]
```
...
```mermaid
flowchart TD
    N1["Capa normal: tenemos metodo, plantillas y 13 leads reales, cero mensajes mandados"] --> T1["Capa tomista: esto es potencia perfectamente dispuesta al acto - no falta forma, falta el acto de la voluntad que la ejecute"]
    T1 --> T2["Aquino: la prudencia bien informada que no se traduce en accion no es prudencia completa, es solo su primer momento"]
    T2 --> B1["Santiago 2:26 - el cuerpo sin espiritu esta muerto, asi tambien la fe sin obras. El metodo sin el primer mensaje enviado es cuerpo sin espiritu"]
    B1 --> R1["Resolucion: el primer envio real de hoy, aunque sea a una sola empresa, convierte toda esta simulacion en acto"]
```
...
```mermaid
flowchart TD
  A[Perfil encontrado en LinkedIn] --> B{Es una persona individual publicando contenido propio?}
  B -- Si --> C{El contenido es de finanzas, manufactura o IA y suena genuino?}
  C -- Si --> D[Motor 1: comentar primero, 2-3 comentarios, luego conexion]
  C -- No --> H[Descartar o guardar para despues]
  B -- No, es empresa --> E{Hay senal de necesidad de contratacion o automatizacion?}
  E -- Si --> F[Motor 2: mensaje directo sobre la vacante detectada]
  E -- No --> G{Hay alguien identificable publicando en nombre propio?}
  G -- Si --> D
  G -- No --> H
```
