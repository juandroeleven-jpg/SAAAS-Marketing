...
35. **Decidir a cuál de las dos columnas Boston pertenece la ficha auditada** — 🆕 la auditoría paralela [`simulacion-38-boston40-verificacion.md`](simulacion-38-boston40-verificacion.md) confirma que **"Boston" y "Boston 4.0" son dos columnas distintas de la misma hoja**, que difieren en 5 filas (certificación, canal para lentes, Quick Visor Release System, ventilaciones y estilo). **La misma pieza da 11/13 contra Boston y 13/13 contra Boston 4.0.** El excel no lo resuelve: hace falta que la pieza lleve el nombre del modelo visible. Mientras tanto conviven los 2 archivos, uno por columna. **Sigue abierto** aparte si **"Evolution 929"** es o no una columna de esta hoja: ninguna de las 2 capturas lo muestra, pese a que la Simulación 16 lo audita como tal.
36. **Corregir la certificación de la ficha del Boston:** dice "DOT / FNVSS 510", el excel dice **"DOT & ECE"**. ⚠️ Ojo: la cadena correcta **no** lleva "22.06" — ese número es de la hoja EDGEPRO, la hoja EDGE nunca lo escribe. Copiarlo sería inventar un dato, igual que "FNVSS 510".
...
**Total: 39 puntos pendientes de confirmación** (37 abiertos + el punto nuevo sin numerar de la falta de ortografía "HOMOLOGACÓN" del caso Hero —cuya **2ª aparición confirmada** es justamente la ficha del Boston, ahora en la otra hoja— + el #19 ya cerrado en la Revisión 2026-07-29 del Stellar, con el #24 cerrado en la del Shift) repartidos en los 11 casos (el detalle completo de cada uno está en la sección "🔴 Pendiente de tu parte" / "Qué hay que hacer" del archivo correspondiente).
...
[`simulacion-33-boston-verificacion.md`](simulacion-33-boston-verificacion.md) — ⚠️ **11 MATCH / 2 MISMATCH / 0 SIN DATO** contra la columna **Boston**; la huella digital (Flip Up + LED) identifica la **familia Boston** sin ambigüedad y descarta las otras 14 columnas del catálogo · ⚠️ **ojo:** contra la columna **Boston 4.0** la misma pieza da **13/13** — ver [`simulacion-38-boston40-verificacion.md`](simulacion-38-boston40-verificacion.md) y el pendiente #35 antes de elegir prompts · 🧭 **primer caso auditado explícitamente contra la hoja EDGE con comparación fila por fila contra la EDGEPRO** · caso hermano de colorways: [`simulacion-21-boston-4-colores.md`](simulacion-21-boston-4-colores.md)
...
🟡 **Ambigüedad de la Escena 1 — pendiente de confirmación del usuario:** el usuario escribió *"como si manejara bici en una montaña"*, pero este es un **casco integral modular de motocicleta** —la mentonera abatible, el visor y el LED trasero son piezas de moto— y un casco así en una bici comunica el producto equivocado. El prompt se escribió para **motocicleta** y el `BLOQUE DE VEHÍCULO` quedó **aislado en un solo párrafo** para poder cambiarlo sin tocar el resto. **Nadie eligió todavía.**

🟡 **Ambigüedad de la Escena 2 — pendiente de confirmación del usuario:** el usuario dijo *"lifestyle"* pero describió un **set de estudio**, y además pidió *"la misma posición y todo"*. **(A) el casco solo** queda como principal porque es la única lectura que permite conservar **literalmente** la vista lateral de la foto de producto; **(B) con una persona** queda como variante, donde el ángulo pasa a depender de la pose de la cabeza. **Nadie eligió todavía.**
...
| Prompt / pieza | Tipo de pieza | Estado | Ubicación |
|---|---|---|---|
| Escena 1 — Lifestyle en la montaña, de día, conduciendo (visor opaco, rostro no visible) | Tipo A (traslado de escena, geometría intacta) | 🔴 bloqueado — el texto del prompt ya está listo para correr; falta confirmar **moto vs. bici** (se cambia solo el `BLOQUE DE VEHÍCULO`) | `simulacion-37-lifestyle-montana-visor-opaco.md` |
...
🟡 **Ambigüedad de la Foto 2 — pendiente de confirmación del usuario:** la frase *"un hombre mirando de frente que (no) se vea por debajo del visor"* admite dos lecturas. El prompt se escribió con la lectura que el propio usuario cerró después —*"que el visor sea completamente negro y no se vea a través de él"*, o sea **visor totalmente opaco y rostro no visible**—. Queda pendiente confirmar si además quería **ver el mentón o la boca** por debajo de la línea del visor: **si confirma esa lectura, el prompt no se puede parchear**, porque con la mentonera **cerrada** eso es físicamente imposible — habría que **partir de la referencia LATERAL con la mentonera abierta** y rehacer el encuadre. **Nadie eligió todavía.**
...
| Foto 2 — Centro comercial, hombre de frente, **visor 100 % opaco** y **mentonera CERRADA** (referencia FRONTAL) | Tipo A (traslado de escena + cambio de opacidad de una pieza) | 🔴 bloqueado — el texto del prompt ya está listo para correr; falta confirmar la ambigüedad de *"que (no) se vea por debajo del visor"* | `simulacion-39-lifestyle-avenida-y-centro-comercial.md` |
...
| Casco modular visor opaco + LED (2 lifestyle: montaña + estudio LED) | 3 | 0 | 0 | 3 |
| Casco modular en 2 estados (2 lifestyle: avenida + centro comercial) | 2 | 1 | 0 | 1 |
| **Total** | **92** | **62** | **10** | **15** |
