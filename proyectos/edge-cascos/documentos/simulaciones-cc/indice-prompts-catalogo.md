...
### Boston — [`simulacion-33-boston-verificacion.md`](simulacion-33-boston-verificacion.md)
35. ~~**Decidir a cuál de las dos columnas Boston pertenece la ficha auditada.**~~ ✅ **CERRADO (2026-07-29, confirmado por el usuario).** Respuesta textual: ***"yo te mandé a hacer 2 diferentes, una del Boston y la otra del Boston 4.0."*** No había una sola pieza cuya columna hubiera que adivinar: **son DOS fichas separadas, una por modelo**. [`simulacion-33`](simulacion-33-boston-verificacion.md) queda como la ficha del **Boston** (**11 MATCH / 2 MISMATCH**, con sus 2 ítems de reemplazo) y [`simulacion-38`](simulacion-38-boston40-verificacion.md) como la del **Boston 4.0** (**13 MATCH / 0 MISMATCH**, sin reemplazos). Las dos columnas siguen existiendo en la misma hoja y difieren en 5 filas (certificación, canal para lentes, Quick Visor Release System, ventilaciones y estilo). 🔎 **Queda como hallazgo de método, no como duda:** las dos fichas son **visualmente idénticas** y **el veredicto cambia según el modelo al que se las atribuya** — **refuerza el pedido de que cada pieza lleve el nombre del modelo visible**. **Sigue abierto** aparte si **"Evolution 929"** es o no una columna de esta hoja: ninguna de las 2 capturas lo muestra, pese a que la Simulación 16 lo audita como tal.
36. ~~**Corregir la certificación de la ficha del Boston.**~~ ✅ **CERRADO (2026-07-29, confirmado por el usuario)** en cuanto al número de norma. Respuesta textual: ***"dot fmvss"***. El banner lleva **"DOT"** grande y, debajo y más chico, **"FMVSS 218"**. ⚠️ Ojo con las dos mitades: **(a)** el número — **"FNVSS 510" está mal por partida doble**, las letras (**FMVSS**, no FNVSS) y el número (**218**, no 510) — y **(b)** el valor de certificación de la primera línea, que se sigue leyendo de la columna de cada modelo: para el **Boston** es **"DOT & ECE"**, **sin "22.06"** (ese número es de la hoja EDGEPRO, la hoja EDGE nunca lo escribe), y para el **Boston 4.0** es **"DOT"** a secas, sin "& ECE". Ver el punto de alcance de template maestro más abajo.
37. ~~**Confirmar los 2 ítems de reemplazo elegidos:** "Interior EPS de alta resistencia" en la tarjeta de homologación y "Material exterior ABS alta resistencia" en el grid.~~ ✅ **CERRADO (2026-07-29) — confirmados en la práctica, no por respuesta sino por resultado.** La **pieza generada** con los 2 prompts llegó y **lleva los 2 reemplazos**, con lo que la ficha pasó de **11/13 a 13/13**. El suplente "4 VENTILACIONES" queda descartado y ya no hace falta.
38. Subir la ficha real del Boston, **la pieza generada** y la hoja "FICHA DE CASCOS" como adjunto real para versionarlas.
38b. 🔁 **Volver a correr el Prompt A del Boston — SIN cambiarlo.** La pieza generada salió con el banner **"DOT / FNVSS 510"**, o sea que el prompt **aplicó la corrección de contenido y se salteó la de certificación**; ya contempla la corrección completa (**"DOT" + "& ECE" + "FMVSS 218"**, sin "22.06" y sin "FNVSS 510"), así que **falló la aplicación, no la instrucción**. El Prompt B **no** hay que volver a correrlo por este motivo.
38c. 👁️ **Verificar a ojo dos cosas de la pieza generada que la captura recibida no permite confirmar:** **(a)** ¿el ícono de **HEBILLA MICROMÉTRICA** conserva la **X roja** o salió limpio? El excel confirma la feature con **X**, así que tiene que estar limpio; si la X está, se vuelve a correr el Prompt B. **(b)** ¿la franja de título dice **"HOMOLOGACIÓN"** (bien escrito) o **"HOMOLOGACÓN"** (como la referencia)? El recorte recibido no muestra esa franja. **Ninguno de los dos se da por bueno ni por malo hasta esa verificación.**

> 🧭 **Hallazgo estructural de nivel catálogo (Boston) — no es un pendiente de este caso, es una regla de proceso:** el catálogo tiene **dos hojas de excel con sets de filas distintos**. La hoja **EDGE** ("FICHA DE CASCOS") tiene una fila **PRECIO** que la **EDGEPRO** no tiene, y **le faltan** las filas **KIT DE MECANISMO VISOR** y **CON MALETÍN DE LUJO** que la EDGEPRO sí tiene; además, la **certificación es por modelo y no por marca** (conviven "DOT" y "DOT & ECE" en la misma hoja, y nunca aparece el "22.06"). Consecuencia: **la misma plantilla de ficha se está aplicando a dos líneas de producto con sets de specs distintos**, lo cual explica por qué los mismos claims fallan una y otra vez en modelos distintos. Consecuencia operativa inmediata: **el relleno estándar del grid ("Kit de mecanismo visor") no existe para los modelos EDGE** — es exactamente el callejón contra el que chocó Shanghai (#18). Ya es ítem del checklist Tipo C en `orquestacion-agentes-paralelos.md`.

### Boston 4.0 — [`simulacion-38-boston40-verificacion.md`](simulacion-38-boston40-verificacion.md)
39. ~~**Confirmar la segunda línea del banner de certificación** — el Prompt A la eliminaba y dejaba solo "DOT".~~ ✅ **CERRADO (2026-07-29, confirmado por el usuario):** ***"dot fmvss"***. El banner lleva **"DOT"** grande y **"FMVSS 218"** debajo, ya aplicado en el Prompt A. La primera línea sigue siendo **"DOT"** a secas —lo que dice la columna Boston 4.0— y **no se le agrega "& ECE"**.
40. Subir la ficha real del Boston 4.0 y la captura de la hoja "FICHA DE CASCOS" con las 9 columnas como adjunto real, para versionarlas.

> ⚖️ **CORRECCIÓN DE NIVEL CATÁLOGO — ✅ confirmada por el usuario el 2026-07-29: "FNVSS 510" → "FMVSS 218" en el TEMPLATE MAESTRO.** Respuesta textual: ***"dot fmvss"***. **"FNVSS 510" está equivocado por partida doble:** las **letras** (es **FMVSS**, *Federal Motor Vehicle Safety Standard*, no "FNVSS") y el **número** (es **218**, la norma de cascos de motociclismo, no "510"). **Evidencia:** los **stickers de certificación pegados en los cascos físicos** de la marca, visibles en las **fotos de producto del propio catálogo** (en el visor y en la nuca), que dicen **"DOT · FMVSS NO. 218 · CERTIFIED"**. No es una discrepancia entre dos fuentes: "FNVSS 510" es un dato que **no existe en ninguna** — ninguna de las dos hojas del excel lo escribe nunca. **Alcance:** la corrección **vale para el template maestro de todo el catálogo**, no solo para Boston y Boston 4.0. El defecto viene de la **plantilla genérica de marketing** y ya apareció en **Kratos (#3), Vortex (#10), Shanghai (#16), Shift (#23), Evolution 929 (#27), Hero, Carbex, Boston (#36) y Boston 4.0 (#39)** — o sea en **las dos hojas y las dos líneas de producto**. Se arregla **una sola vez en el template**, igual que la falta "HOMOLOGACÓN" y el tache heredado del ícono de hebilla micrométrica; corregirlo ficha por ficha es repetir nueve veces el mismo parche. ⚠️ **Lo que NO cambia:** el **valor de certificación de la primera línea** se sigue leyendo de la fila *Certificación* de la **columna de cada modelo** ("DOT & ECE" en Boston y Shanghai, "DOT" a secas en Boston 4.0 y Evolution 929, "DOT & ECE 22.06" en la hoja EDGEPRO). **No se hereda por marca y no se inventa "& ECE" donde el excel no lo dice.**

> 🔒 **Caso cerrado (Boston) — el tache del ícono de "hebilla micrométrica" viene de la plantilla:** con el Boston son **tres apariciones del mismo arte con veredictos distintos** — **Stellar** (excel = X → tache **falso**, #20), **Shift** (excel = N/A → tache **correcto por casualidad**, #24) y **Boston** (excel = X → tache **falso**). El mismo tache aparece en modelos con X y con N/A, en **las dos hojas** y en **las dos líneas de producto**: **no se decidió nunca por modelo**. **Qué hay que hacer:** revisarlo **modelo por modelo en las dos hojas** y decidir por dato — ícono limpio si la celda dice X, y solo con marca de negación si dice N/A.

### ✅ Cerrados el 2026-07-29 por respuesta del usuario — los 4 casos de ilustración

**No se borran: el historial de qué se preguntó y qué se respondió es parte del valor de este documento.** Cada uno con la respuesta textual.

41. ~~**Casco jet Bob Esponja (Sim. 31) — qué es "el palo".**~~ ✅ **CERRADO (2026-07-29):** ***"el palo es lo que se ve dentro del visor del casco."*** Se **descarta** la lectura de la línea vertical punteada de maquetación —que ahora queda explícitamente en la lista de lo que **no se toca**— y lo que hay que eliminar es un **objeto con forma de vara / palo que se ve DENTRO del visor**. El `CAMBIO 2` se reescribió **integrado con el `CAMBIO 1`**: como el bloque `QUÉ SE VE A TRAVÉS DEL VISOR` ya declaraba en positivo que ahí va **interior de casco vacío y nada más**, el palo quedó sumado a las prohibiciones de esa zona junto a cara/cabeza/persona/maniquí. **El caso sale de 🔴 bloqueado.** 🔎 *Hallazgo de método:* **la ambigüedad se resolvió sola al integrar los dos cambios** — argumento a favor de declarar el contenido de una zona **en positivo** antes que ir prohibiendo objeto por objeto; no es lección nueva, es un beneficio no anticipado de la lección de transparencia que ya deja ese archivo.
42. ~~**Lifestyle cafetería (Sim. 34) — "sentado con este casco": ¿casco sobre la mesa (A) o puesto (B)?**~~ ✅ **CERRADO (2026-07-29):** ***"el casco en la cafetería es sobre una persona."*** Gana **(B)**, que pasa a ser el prompt **definitivo y único**; **la variante (A) se eliminó del documento**, no se archivó. ⚠️ **Consecuencia registrada:** (A) era la lectura que conservaba **literalmente** el ángulo de 3/4 delantero desde arriba de la foto de producto, así que con el casco puesto **el ángulo pasa a depender de la pose de la cabeza** — es ahora el punto de riesgo principal del caso. Se resolvió con la regla ya establecida en el repo (**el ángulo del casco lo define la foto de producto, no la pose del modelo**), reforzando ese bloque y moviendo su chequeo al **puesto 1** de la verificación final; se sumó además el bloque de **rostro no visible**. **Los 2 prompts salen de 🔴 bloqueado y quedan en 1 solo ✅ listo.**
43. ~~**Lifestyle centro comercial (Sim. 39, Foto 2) — "que (no) se vea por debajo del visor".**~~ ✅ **CERRADO (2026-07-29):** ***"rostro totalmente tapado con el casco en el centro."*** Confirma la lectura con la que el prompt **ya estaba escrito** —**visor 100 % opaco, rostro no visible, mentonera cerrada**— y **el prompt no cambió**. Se descarta la lectura de ver el mentón por debajo del visor, que además era **físicamente imposible** con la mentonera cerrada. **El caso sale de 🔴 bloqueado.**
44. ~~**Lifestyle montaña (Sim. 37, Escena 1) — ¿moto o bici?**~~ ✅ **CERRADO (2026-07-29):** ***"moto en la montaña."*** Confirma la lectura con la que el prompt **ya estaba escrito** y **el prompt no cambió**. **El caso sale de 🔴 bloqueado.** ⚠️ **Ojo:** la **Escena 2** del mismo archivo (cuarto oscuro con LED rojo y azul, **casco solo (A) vs. con persona (B)**) **sigue abierta** — el usuario no la respondió.

*(Los otros 2 puntos cerrados el mismo día son los **#35 y #36** del Boston y el **#39** del Boston 4.0, marcados arriba en su propio bloque.)*

**Total: 46 puntos registrados** (se sumaron el **#38b** y el **#38c** del Boston el 2026-07-29, al llegar la pieza generada), de los cuales **8 ya están cerrados** (#19 Stellar, #24 Shift, #35, #36 y **#37** Boston, #39 Boston 4.0, y los #41 a #44 de los casos de ilustración cerrados el 2026-07-29 — el #36 y el #39 son la misma decisión de certificación vista desde los dos archivos, y el **#37 se cerró por resultado y no por respuesta**: la pieza generada llegó con los 2 reemplazos y dio 13/13). Quedan **abiertos ~36 puntos** repartidos en los 13 casos, más el punto sin numerar de la falta de ortografía "HOMOLOGACÓN" del caso Hero —cuya **2ª aparición confirmada** es la ficha del Boston, ahora en la otra hoja—. El detalle completo de cada uno está en la sección "🔴 Pendiente de tu parte" / "Qué hay que hacer" del archivo correspondiente.
...
[`simulacion-33-boston-verificacion.md`](simulacion-33-boston-verificacion.md) — 🟠 **PIEZA GENERADA AUDITADA (2026-07-29): 13 MATCH / 0 MISMATCH / 0 SIN DATO.** La ficha original daba **11/13**; la **pieza regenerada con los 2 prompts de este archivo** da **13/13** — los 2 reemplazos (*INTERIOR EPS DE ALTA RESISTENCIA* y *MATERIAL EXTERIOR ABS ALTA RESISTENCIA*) taparon los 2 MISMATCH duros y **son la prueba literal de que la pieza es el resultado de estos prompts y no la ficha original** (esos 2 ítems no existen en la plantilla genérica). **Primer caso del catálogo donde una ficha con mismatches se regenera y cierra al 100 %.** ❌ **Único defecto pendiente:** el banner salió **"DOT / FNVSS 510"** en vez de **"DOT & ECE" + "FMVSS 218"** — **la corrección de contenido se aplicó y la de certificación no** → hay que **volver a correr el Prompt A tal como está**, que ya la contempla. 👁️ **Pendiente de verificación visual del usuario:** si el ícono de *hebilla micrométrica* conserva la X roja y si el título dice "HOMOLOGACIÓN" o "HOMOLOGACÓN" — la captura recibida no permite confirmar ninguno de los dos · ✅ **cerrado el 2026-07-29 por el usuario:** *"yo te mandé a hacer 2 diferentes, una del Boston y la otra del Boston 4.0"* — **son dos fichas distintas**, esta es la del **Boston** y la del Boston 4.0 está en [`simulacion-38-boston40-verificacion.md`](simulacion-38-boston40-verificacion.md) (13/13, sin reemplazos). 🔎 **Queda como hallazgo de método:** las dos fichas son **visualmente idénticas** y el veredicto cambia según el modelo — refuerza el pedido de que cada pieza lleve el nombre del modelo visible · 🧭 **primer caso auditado explícitamente contra la hoja EDGE con comparación fila por fila contra la EDGEPRO** · caso hermano de colorways: [`simulacion-21-boston-4-colores.md`](simulacion-21-boston-4-colores.md)

| Prompt / pieza | Tipo de pieza | Estado | Ubicación |
|---|---|---|---|
| Prompt A — Tarjeta de homologación Boston (7 ítems, 7/7 confirmados) | Homologación (Tipo B) | 🟠 **CORRIDO (2026-07-29) — resultado a medias, hay que volver a correrlo SIN cambiarlo.** ✅ Aplicó la corrección de **contenido**: la lista salió con los 7 ítems correctos, incluido el reemplazo *INTERIOR EPS DE ALTA RESISTENCIA* → **7/7 MATCH**. ❌ **NO aplicó la corrección de certificación**: el banner salió **"DOT / FNVSS 510"**, copiado tal cual de la referencia, en vez de **"DOT" + "& ECE" + "FMVSS 218"** — pese al bloque `CRÍTICO — CERTIFICACIÓN EXACTA` y a su chequeo propio en la verificación final. **El prompt no se toca: falló la aplicación, no la instrucción.** 👁️ El título ("HOMOLOGACIÓN" vs. "HOMOLOGACÓN") **no es verificable** en la captura recibida. Saca "Sistema de liberación rápida del visor" (N/A) y lo reemplaza por **"Interior EPS de alta resistencia"**; banner corregido a **"DOT"** + **"& ECE"** (valor de la columna Boston, sin "22.06") + **"FMVSS 218"** en lugar de "FNVSS 510"; título **"HOMOLOGACIÓN"** bien escrito. Incluye lienzo constante en números, geometría de separadores, conteo forzado de ítems y prohibición de adornos | [`simulacion-33-boston-verificacion.md`](simulacion-33-boston-verificacion.md), sección `## 5. Prompts corregidos` |
| Prompt B — Grid de íconos Boston (2x3, 6/6 confirmados, sin tache) | Grid íconos (Tipo B) | ✅ **CORRIDO (2026-07-29) — contenido correcto: 6/6 MATCH.** Saca "Canal para lentes" (N/A) y lo reemplaza por **"Material exterior ABS alta resistencia"**, que salió **completo y sin truncar**; **conserva "Diseño modular" y "Con luz LED"**, que acá son datos reales; 6 celdas exactas en 2x3, sin duplicados. ⚠️ **Dos salvedades:** 👁️ el **tache rojo** sobre "Hebilla micrométrica" (feature confirmada con X) **no se distingue en la captura** → pendiente de verificación visual del usuario, y si sigue ahí hay que volver a correrlo; y 🔀 las **celdas 4 y 5 salieron intercambiadas** respecto del orden declarado en el prompt (observación menor, no cambia el veredicto) | [`simulacion-33-boston-verificacion.md`](simulacion-33-boston-verificacion.md), sección `## 5. Prompts corregidos` |

### Boston 4.0 (Simulación 38) — Tipo C, ficha vs. excel (hoja "FICHA DE CASCOS", marca EDGE)
[`simulacion-38-boston40-verificacion.md`](simulacion-38-boston40-verificacion.md) — ✅ **13 MATCH / 0 MISMATCH / 0 SIN DATO: primera ficha 13/13 del catálogo.** No hay que reemplazar ningún ítem; solo se corrigen **3 defectos de arte y de texto**. 🔎 **Hallazgo principal:** la plantilla de ficha **no estaba mal diseñada sino mal asignada** — fue escrita para un casco con este perfil de features (flip up + LED + liberación rápida de visor) y después se copió sobre modelos que no las tienen, lo que explica de una sola vez casi todos los mismatches acumulados del catálogo. ✅ **Confirmado el 2026-07-29:** es una **ficha distinta** de la del [Boston (Sim. 33)](simulacion-33-boston-verificacion.md), aunque las dos piezas sean visualmente idénticas.

| Prompt / pieza | Tipo de pieza | Estado | Ubicación |
|---|---|---|---|
| Prompt A — Tarjeta de homologación Boston 4.0 (7 ítems, **sin cambios de contenido**) | Homologación (Tipo B) | ✅ listo para usar — todavía no corrido. Reproducción fiel: los 7 ítems no cambian. Corrige el título **"HOMOLOGACÓN" → "HOMOLOGACIÓN"** y el banner, que pasa a **"DOT"** grande + **"FMVSS 218"** debajo (antes eliminaba la 2ª línea; ✅ decidido por el usuario el 2026-07-29). **Sin "& ECE"**: la columna de este modelo dice "DOT" a secas | [`simulacion-38-boston40-verificacion.md`](simulacion-38-boston40-verificacion.md), sección `## Prompts corregidos` |
| Prompt B — Grid de íconos Boston 4.0 (2x3, 6 celdas, **sin cambios de contenido**, sin tache) | Grid íconos (Tipo B) | ✅ listo para usar — todavía no corrido. Reproducción fiel: los 6 ítems no cambian. Único cambio: **sacar el tache rojo** del ícono de "Hebilla micrométrica" (el excel la confirma con X), con borrado y reconstrucción del dibujo que quedaba debajo | [`simulacion-38-boston40-verificacion.md`](simulacion-38-boston40-verificacion.md), sección `## Prompts corregidos` |
...
## Casco jet arte Bob Esponja — visor transparente + quitar "el palo" del visor — Simulación 31

Casco abierto/jet, vista frontal, calota amarillo lima con arte grafiti tipo Bob Esponja, visor grande ahumado y fondo de plantilla con formas geométricas grises tenues. Edición puntual de **2 cambios** sobre una foto ya aprobada: el visor pasa de ahumado a transparente (revelando el interior del casco **vacío**) y se elimina **un objeto con forma de vara que se ve DENTRO del visor**. Tipo A (edición puntual) con componente Tipo D (limpieza). Caso hermano de la Simulación 6a, en un archivo propio porque aquella es una auditoría de Etapa 1 y esta es una edición.

✅ **Desbloqueado (2026-07-29, confirmado por el usuario):** *"el palo es lo que se ve dentro del visor del casco"*. Se descartó la lectura de la **línea vertical punteada de maquetación**, que ahora queda explícitamente en la lista de lo que **no se toca** (si se la quiere sacar, es un pedido aparte). Los **2 cambios se reescribieron integrados**: como el bloque `QUÉ SE VE A TRAVÉS DEL VISOR` ya declaraba en positivo que ahí va **interior de casco vacío y nada más**, el palo se sumó a las prohibiciones de esa zona junto a cara/cabeza/persona/maniquí. 🔎 **La ambigüedad se resolvió sola al integrar los dos cambios** — argumento a favor de declarar el contenido de una zona en positivo antes que ir prohibiendo objeto por objeto (beneficio no anticipado de la lección de transparencia que ya deja este archivo, no lección nueva).

[`simulacion-31-casco-bob-esponja-visor-transparente.md`](simulacion-31-casco-bob-esponja-visor-transparente.md)

| Prompt / pieza | Tipo de pieza | Estado | Ubicación |
|---|---|---|---|
| Edición puntual — visor ahumado → transparente + quitar el palo que se ve dentro del visor | Tipo A (edición puntual) + D (limpieza) | ✅ listo para usar — todavía no corrido. **Desbloqueado el 2026-07-29**: la interpretación de "el palo" quedó confirmada por el usuario y el prompt está reescrito con ella | `simulacion-31-casco-bob-esponja-visor-transparente.md` |
...
✅ **Ambigüedad CERRADA (2026-07-29, confirmada por el usuario): el casco va PUESTO.** Textual: *"el casco en la cafetería es sobre una persona"*. Gana la lectura **(B)**, que queda como **prompt definitivo y único**; **la variante (A) —casco apoyado sobre la mesa— se eliminó del documento**, no se archivó como alternativa. ⚠️ **Consecuencia registrada:** (A) era la lectura que conservaba **literalmente** el ángulo de 3/4 delantero desde arriba de la foto de producto —un objeto apoyado mantiene su ángulo solo—, así que con el casco puesto **el ángulo pasa a depender de la pose de la cabeza** y es ahora **el punto de riesgo principal del caso**. Se resolvió aplicando la regla ya establecida en el repo —**el ángulo del casco lo define la foto de producto, no la pose del modelo**— y **reforzando ese bloque**: declara el orden de construcción en dos pasos (primero el casco, después la pose alrededor), avisa que una pose poco habitual es aceptable si eso hace falta para respetar el ángulo, y su chequeo pasó a ser **el número 1** de la verificación final. Se sumó además el bloque de **rostro no visible** declarado en positivo.

[`simulacion-34-lifestyle-cafeteria-casco-negro.md`](simulacion-34-lifestyle-cafeteria-casco-negro.md)

| Prompt / pieza | Tipo de pieza | Estado | Ubicación |
|---|---|---|---|
| Lifestyle en cafetería — hombre sentado con el casco **PUESTO** (definitivo) | Tipo A (traslado de escena, geometría intacta) | ✅ listo para usar — todavía no corrido. **Desbloqueado el 2026-07-29** por confirmación del usuario | `simulacion-34-lifestyle-cafeteria-casco-negro.md` |
| ~~(A) Lifestyle en cafetería — casco apoyado sobre la mesa~~ | — | ⛔ **ELIMINADO** (2026-07-29) — el usuario confirmó que el casco va puesto; el prompt se borró del documento para que nadie lo corra por error | — |
...
## Casco modular / flip up negro liso — 4 vistas: quitar stickers + visor 100% opaco negro + iluminación de set — Simulación 36

Cuatro fotos de producto del **mismo casco integral modular / flip up, negro liso**, sobre fondo blanco, en cuatro vistas (frontal, lateral, trasera, superior). **Un prompt de edición puntual por vista**: quitar el film protector y los stickers donde los hay, volver el visor **100% opaco negro** en las vistas 1 y 2, y emparejar la iluminación en las cuatro. Tipo **D** (limpieza de producto) + **A** (edición puntual, geometría intacta).

**Requisito transversal:** *"que den la misma apreciación"* — las cuatro tienen que leerse como **un mismo set fotográfico**. Por eso el **bloque de iluminación está redactado con las mismas palabras exactas en los cuatro prompts**, y lo único que varía por vista es un addendum corto que nombra el defecto concreto de esa foto.

**El corazón del caso** es el visor opaco: *"full opaco negro, o sea no opaco como si fuera de mentira, sino que no se pueda ver a través de él"*. Es la **operación inversa** de las Simulaciones [31](simulacion-31-casco-bob-esponja-visor-transparente.md) y [32](simulacion-32-visor-transparente-4k-laterales.md), y comparte su trampa: hay que declarar que **el visor sigue existiendo como pieza física** —forma, borde, curvatura, espesor y **reflejos especulares**—, porque el atajo barato del generador para cumplir "que no se vea a través" es pintar un **agujero negro plano**. Se aplica además la **regla de negocio de no incluir stickers de certificación DOT/FMVSS** ([Simulación 30](simulacion-30-edge-racing-livery.md)).

🟡 **3 asunciones pendientes de confirmación del usuario:** (1) el **logo "EDGE®" impreso en la calota se conserva** (branding del producto) y solo se quita el del film del visor; (2) el **acabado real es negro mate** y la trasera se ve más brillante por la luz, no por la pintura; (3) reiluminar entra dentro de "que el casco no cambie nada", entendiendo que lo único que cambia es la **distribución** de la luz.

⚠️ **Correr cada prompt en una sesión aislada** del generador (contaminación cruzada, caso Vortex).

[`simulacion-36-casco-negro-4-vistas-limpieza.md`](simulacion-36-casco-negro-4-vistas-limpieza.md)

| Prompt / pieza | Tipo de pieza | Estado | Ubicación |
|---|---|---|---|
| Vista 1 — FRONTAL: quitar el film protector del visor completo + visor 100% opaco negro + iluminación uniforme | Tipo D (limpieza) + A (edición puntual) | 🔴 bloqueado — el texto del prompt ya está listo para correr; falta confirmar las 3 asunciones (sobre todo el logo de la calota, que afecta a esta vista) | `simulacion-36-casco-negro-4-vistas-limpieza.md` |
| Vista 2 — LATERAL: quitar los restos del film del visor + visor 100% opaco negro + iluminación uniforme | Tipo D (limpieza) + A (edición puntual) | 🔴 bloqueado — el texto del prompt ya está listo para correr; falta confirmar las 3 asunciones | `simulacion-36-casco-negro-4-vistas-limpieza.md` |
| Vista 3 — TRASERA: quitar el escudo "EDGE HELMETS" y la etiqueta "DOT / FMVSS NO. 218" + iluminación uniforme | Tipo D (limpieza) + A (edición puntual) | 🔴 bloqueado — el texto del prompt ya está listo para correr; falta confirmar el acabado real (esta es la vista que se ve más brillante) | `simulacion-36-casco-negro-4-vistas-limpieza.md` |
| Vista 4 — SUPERIOR: solo iluminación (corregir el degradé centro claro → bordes negros) | Tipo A (edición puntual, solo reiluminación) | 🔴 bloqueado — el texto del prompt ya está listo para correr; falta confirmar que reiluminar no cuenta como "cambiar el casco" | `simulacion-36-casco-negro-4-vistas-limpieza.md` |

---

## Casco modular / flip up negro mate con visor opaco y LED rojo trasero — 2 fotos lifestyle (montaña de día + estudio oscuro con LED) — Simulación 37

Casco integral **modular / flip up**, **calota negro MATE** lisa, **visor bajado y ya OPACO**, **placa circular del pivote** con su tornillo, **panel lateral inferior y spoiler en negro BRILLANTE** (contraste mate/brillante), **ventilación superior con listones**, goma negra perimetral y —la pieza que más se pierde— una **luz LED ROJA ENCENDIDA de tres puntos alineados sobre el spoiler trasero**. Vista lateral, mirando a la izquierda. Pedido de **2 fotos lifestyle** sobre la misma foto de producto, con **3 requisitos duros** del usuario: **el casco no cambia nada**, **el visor se mantiene opaco** y **no se ve el rostro**. Tipo A con **traslado de escena**: no se transfiere ningún diseño, lo único que cambia es el entorno y la iluminación.

- **Escena 1 — montaña:** de día, al aire libre, camino/ruta de montaña, la persona conduciendo, con luz **natural, tenue y difusa**, ultrarrealista.
- **Escena 2 — estudio oscuro:** set de fotografía en un **cuarto negro**, con **contraluz LED rojo y azul** detrás, *"la misma posición y todo"*.

✅ **Ambigüedad de la Escena 1 — CERRADA (2026-07-29, confirmada por el usuario): es MOTO.** Textual: *"moto en la montaña"*. Confirma la lectura con la que el prompt **ya estaba escrito** y **el prompt no cambia**. Se descarta la lectura literal de *"como si manejara bici en una montaña"*: este es un **casco integral modular de motocicleta** —la mentonera abatible, el visor y el LED trasero son piezas de moto— y en una bici comunicaría el producto equivocado.

🟡 **Ambigüedad de la Escena 2 — SIGUE ABIERTA, pendiente de confirmación del usuario:** el usuario dijo *"lifestyle"* pero describió un **set de estudio**, y además pidió *"la misma posición y todo"*. **(A) el casco solo** queda como principal porque es la única lectura que permite conservar **literalmente** la vista lateral de la foto de producto; **(B) con una persona** queda como variante, donde el ángulo pasa a depender de la pose de la cabeza. **Nadie eligió todavía** — el usuario respondió la de la Escena 1 el 2026-07-29 pero **no** esta.

**El punto propio de este caso** es la **colisión entre una pieza del producto y un elemento de la escena**: el casco tiene una LED roja en el spoiler y la Escena 2 pide luces LED rojas de contraluz. Los prompts de la Escena 2 llevan un bloque dedicado que declara la LED del spoiler **como pieza física y no como efecto de luz**, y exigen que se distinga de los halos difusos del fondo. De ahí sale la lección nueva del caso para el checklist Tipo A.

⚠️ **Correr cada prompt en una sesión aislada** del generador (contaminación cruzada, caso Vortex).

[`simulacion-37-lifestyle-montana-visor-opaco.md`](simulacion-37-lifestyle-montana-visor-opaco.md)

| Prompt / pieza | Tipo de pieza | Estado | Ubicación |
|---|---|---|---|
| Escena 1 — Lifestyle en la montaña, de día, conduciendo en **MOTO** (visor opaco, rostro no visible) | Tipo A (traslado de escena, geometría intacta) | ✅ listo para usar — todavía no corrido. **Desbloqueado el 2026-07-29**: *"moto en la montaña"* confirmado por el usuario, el prompt no cambió | `simulacion-37-lifestyle-montana-visor-opaco.md` |
| Escena 2 (A) — Set en cuarto negro con LED rojo y azul, **casco solo** (principal) | Tipo A (traslado de escena, geometría intacta) | 🔴 bloqueado — el texto del prompt ya está listo para correr; falta confirmar la lectura (A) vs. (B) | `simulacion-37-lifestyle-montana-visor-opaco.md` |
| Escena 2 (B) — Set en cuarto negro con LED rojo y azul, **con una persona** (variante) | Tipo A (traslado de escena, geometría intacta) | 🔴 bloqueado — no correr por default; solo si el usuario confirma la lectura (B) | `simulacion-37-lifestyle-montana-visor-opaco.md` |

---

## Casco modular / flip up negro mate en DOS ESTADOS — 2 fotos lifestyle (avenida pública con tráfico + centro comercial con visor opaco) — Simulación 39

Mismo casco integral **modular / flip up**, **negro MATE** liso, con **acolchado interior ROJO**, correa con herrajes y toma de aire en la mentonera — pero fotografiado en **DOS referencias distintas y en DOS ESTADOS distintos**, una por foto. Pedido de **2 fotos lifestyle**, cada una con su propia imagen adjunta. Tipo A con **traslado de escena**: no se transfiere ningún diseño, lo único que cambia es el entorno y la iluminación.

- **Foto 1 — avenida pública con tráfico** (referencia **LATERAL**): una mujer, de día, con el **fondo medio borroso pero suave**. **Mentonera LEVANTADA y ABIERTA**, visor principal transparente levantado con ella y **visor interno solar oscuro BAJADO**. El usuario pidió textualmente *"con el mismo ángulo — la mujer debe adaptarse al mismo ángulo del casco"*: la **autoridad de ángulo es la foto de producto** y la pose de la modelo se construye alrededor.
- **Foto 2 — centro comercial** (referencia **FRONTAL**): un hombre mirando de frente, sin que se le vea el rostro. **Mentonera BAJADA y CERRADA**, y el **visor pasa de transparente a 100 % OPACO NEGRO** — el **único cambio permitido** en todo el casco.

**El punto propio de este caso** es que **el estado de la mentonera es distinto en cada foto y no se toca**: cada prompt declara su estado **en positivo**, lo describe físicamente y **prohíbe explícitamente el contrario**, porque es exactamente el tipo de detalle que el generador "normaliza" al valor más común del tipo de producto (un casco integral se ve cerrado). El segundo punto es el **visor opaco de la Foto 2**: 100 % negro, sin que se vea absolutamente nada detrás —ni el interior, ni el acolchado rojo, ni la cara, ni una silueta— y a la vez **pieza física real** con forma, borde, curvatura, espesor y **reflejos especulares del centro comercial**. Es la lección de la [Simulación 31](simulacion-31-casco-bob-esponja-visor-transparente.md) y la [Simulación 32](simulacion-32-visor-transparente-4k-laterales.md) aplicada **al revés**, referenciada dentro del prompt.

✅ **Ambigüedad de la Foto 2 — CERRADA (2026-07-29, confirmada por el usuario).** Textual: *"rostro totalmente tapado con el casco en el centro"*. Confirma la lectura con la que el prompt **ya estaba escrito** —**visor 100 % opaco, rostro no visible, mentonera cerrada**— y **el prompt no cambia**. Se descarta la lectura de **ver el mentón o la boca** por debajo de la línea del visor, que además era **físicamente imposible** con la mentonera cerrada de la referencia frontal (ahí la boca y el mentón están tapados por la propia pieza del casco).

⚠️ **Correr cada prompt en una sesión aislada** del generador, y **no en el mismo hilo** — las dos imágenes son el mismo casco en **estados opuestos**, así que el riesgo de contaminación cruzada (caso Vortex) es mayor que de costumbre. Ojo también con **adjuntar la referencia correcta a cada prompt**: es el error más barato de cometer y el prompt no lo puede atrapar.

[`simulacion-39-lifestyle-avenida-y-centro-comercial.md`](simulacion-39-lifestyle-avenida-y-centro-comercial.md)

| Prompt / pieza | Tipo de pieza | Estado | Ubicación |
|---|---|---|---|
| Foto 1 — Avenida pública con tráfico, mujer, **mentonera LEVANTADA** (referencia LATERAL) | Tipo A (traslado de escena, geometría intacta) | ✅ listo para usar — todavía no corrido. Adjuntar la referencia **lateral**, correr en sesión aislada | `simulacion-39-lifestyle-avenida-y-centro-comercial.md` |
| Foto 2 — Centro comercial, hombre de frente, **visor 100 % opaco** y **mentonera CERRADA** (referencia FRONTAL) | Tipo A (traslado de escena + cambio de opacidad de una pieza) | ✅ listo para usar — todavía no corrido. **Desbloqueado el 2026-07-29**: *"rostro totalmente tapado"* confirmado por el usuario, el prompt no cambió. Adjuntar la referencia **frontal**, correr en sesión aislada de la Foto 1 | `simulacion-39-lifestyle-avenida-y-centro-comercial.md` |
...
| Casco jet Bob Esponja (visor transparente) | 1 | 1 | 0 | 0 |
| Casco negro mate visor dorado (lifestyle cafetería) | 2 | 1 | 0 | 0 |
| Casco modular/flip up (4 colorways, goma negra) | 4 | 4 | 0 | 0 |
| Boston — ficha vs. excel (hoja EDGE) | 2 | 2 | 0 | 0 |
| **Boston 4.0 — ficha vs. excel (hoja EDGE)** | **2** | **2** | **0** | **0** |
| Casco negro liso 4 vistas (stickers + visor opaco + set) | 4 | 0 | 0 | 4 |
| Casco modular visor opaco + LED (2 lifestyle: montaña + estudio LED) | 3 | 1 | 0 | 2 |
| Casco modular en 2 estados (2 lifestyle: avenida + centro comercial) | 2 | 2 | 0 | 0 |
| **Total** | **94** | **68** | **10** | **10** |

---

*(los prompts ⛔ SUPERADOS del Stellar y del Shift se cuentan en la columna "Prompts indexados" pero no en ✅/⚠️/🔴, mismo criterio que los ⛔ de Hero — y desde el 2026-07-29 también la variante ⛔ ELIMINADA (A) de la cafetería.)*

**🔴 Bloqueados: de 15 a 10 (2026-07-29).** Los 5 que salieron son los de los 4 casos que el usuario destrabó con sus respuestas: **Bob Esponja** (1), **cafetería** (2, que además se consolidan en 1 solo prompt vigente), **montaña Escena 1** (1) y **centro comercial Foto 2** (1). Los **10 que quedan** son: **Shanghai** (1, grid en 4/6 ítems), **Hero** (1), **casco carbono glossy** (2, división de color sin confirmar), **casco negro liso 4 vistas** (4, las 3 asunciones) y **montaña Escena 2** (2, casco solo vs. con persona — la única ambigüedad de esta tanda que el usuario **no** respondió).

---

**Última actualización:** 2026-07-29 · **se aplicaron 6 decisiones del usuario que estaban abiertas** y se agregó la sección de la **Simulación 38 (Boston 4.0)**; el total pasa de 92 a **94 prompts indexados** y los **🔴 bloqueados bajan de 15 a 10**. Las 6 decisiones, con su respuesta textual: **(1) Boston y Boston 4.0 son DOS fichas distintas** (*"yo te mandé a hacer 2 diferentes, una del Boston y la otra del Boston 4.0"*) — la Sim. 33 queda como la del **Boston** (11/13, con sus 2 reemplazos) y la Sim. 38 como la del **Boston 4.0** (13/13, sin reemplazos); se sacó de los dos archivos el bloque de "no se sabe cuál de las dos columnas es" y **queda como hallazgo de método** que las dos fichas son **visualmente idénticas** y que el veredicto cambia según el modelo, lo que **refuerza el pedido de que cada pieza lleve el nombre del modelo visible** (pendientes #35 y #36 cerrados). **(2) La certificación va "DOT / FMVSS 218"** (*"dot fmvss"*) — "FNVSS 510" estaba mal por las **letras** y por el **número**, y la evidencia son los **stickers de certificación de los cascos físicos**, visibles en las fotos de producto del propio catálogo; el Prompt A del Boston 4.0, que **eliminaba** la segunda línea del banner, ahora lleva "DOT" + "FMVSS 218", y el del Boston lleva "DOT" + "& ECE" + "FMVSS 218". **La corrección vale para el TEMPLATE MAESTRO de todo el catálogo**, con su bloque propio en la sección de pendientes; **no cambia** el valor de la primera línea, que se sigue leyendo de la columna de cada modelo (nada de inventar "& ECE" donde el excel no lo dice). **(3) "El palo" del Bob Esponja está DENTRO del visor** (*"el palo es lo que se ve dentro del visor del casco"*) — se descartó la línea punteada de maquetación y el `CAMBIO 2` se reescribió **integrado con el `CAMBIO 1`**, porque la zona ya estaba declarada en positivo como interior de casco vacío: **la ambigüedad se resolvió sola al integrar los dos cambios**. **(4) El casco de la cafetería va PUESTO** (*"el casco en la cafetería es sobre una persona"*) — gana la variante (B), la (A) **se eliminó**, y como (A) era la lectura que conservaba literalmente el ángulo de la foto de producto, se **reforzó el bloque de "el ángulo del casco lo define la foto de producto, no la pose del modelo"**, que pasa a ser el chequeo #1 de la verificación final. **(5) En el centro comercial el rostro va totalmente tapado** (*"rostro totalmente tapado con el casco en el centro"*) — confirma la lectura ya escrita, **el prompt no cambia**. **(6) En la montaña es MOTO** (*"moto en la montaña"*) — confirma la lectura ya escrita, **el prompt no cambia**. ⚠️ **Sigue abierta** la ambigüedad de la **Escena 2 de la Simulación 37** (casco solo vs. con persona), que el usuario no respondió. · *(registro previo del mismo día)* agregada la sección de la **Simulación 39** (mismo casco modular / flip up negro mate fotografiado en **DOS referencias y DOS ESTADOS distintos**: **2 fotos lifestyle**, avenida pública con tráfico y centro comercial): **2 prompts** —Foto 1, avenida con la **mentonera LEVANTADA** desde la referencia **lateral**, ✅ lista para usar; Foto 2, centro comercial con la **mentonera CERRADA** y el **visor pasando a 100 % opaco negro** desde la referencia **frontal**, 🔴 bloqueada—; el total pasa de 90 a **92** prompts indexados. La diferencia estructural del caso es que **el estado de la mentonera cambia entre las dos fotos y no se toca**: cada prompt lo declara **en positivo**, lo describe físicamente y **prohíbe explícitamente el estado contrario**, y los dos se corren en **sesiones aisladas** porque el riesgo de contaminación cruzada es mayor de lo habitual (es el mismo casco en configuraciones opuestas). El **acolchado interior ROJO** tiene tratamiento propio en los dos prompts, con la aclaración de que en la Foto 2 dejar de verse **a través del visor opaco** es correcto pero desaparecer del **borde inferior** no lo es. Queda **1 ambigüedad pendiente de confirmación del usuario**: la frase *"que (no) se vea por debajo del visor"* se escribió con la lectura que el propio usuario cerró después (*"visor completamente negro y que no se vea a través de él"*), y quedó anotado que la otra lectura **no se puede parchear** —con la mentonera cerrada ver el mentón es físicamente imposible— y obligaría a partir de la referencia lateral. Deja **2 lecciones nuevas** para el checklist Tipo A: (a) **una pieza que admite dos ESTADOS** —mentonera abierta o cerrada, visor arriba o abajo— hay que declararla en cada prompt, porque el generador la **normaliza al valor más común del tipo de producto** aunque la referencia muestre el otro, con el agravante de que si el pedido son varias fotos en estados distintos cada prompt tiene que **prohibir el estado ajeno** y correrse aislado; y (b) si la referencia es una **lámina de catálogo**, un **producto vecino que asoma en el borde** puede incorporarse como **objeto real** de la escena nueva —es plausible y el generador no lo clasifica como artefacto—, así que hay que nombrarlo, enumerar las formas en que podría colarse y cerrar con **conteo forzado** ("contá los cascos: tiene que haber exactamente UNO"). · agregada la sección de la **Simulación 37** (casco modular / flip up negro mate con **visor ya opaco** y **luz LED roja de tres puntos sobre el spoiler**: **2 fotos lifestyle** sobre la misma foto de producto): **3 prompts** —Escena 1, montaña de día conduciendo; Escena 2 (A), set en cuarto negro con contraluz LED rojo y azul, casco solo, principal; Escena 2 (B), la misma con una persona, variante—, los tres 🔴 bloqueados por **2 ambigüedades pendientes de confirmación del usuario**: (1) el usuario escribió *"como si manejara bici"* pero el producto es un **casco integral modular de moto**, así que la Escena 1 se escribió para **motocicleta** con el `BLOQUE DE VEHÍCULO` aislado para poder cambiarlo; (2) el usuario dijo *"lifestyle"* pero describió un **set de estudio** y pidió *"la misma posición y todo"*, así que (A) el casco solo queda como principal por ser la única lectura que conserva literalmente la vista lateral. **Nadie eligió ninguna de las dos.** El total pasa de 87 a **90** prompts indexados. Los **3 requisitos duros** del usuario —casco intacto, **visor opaco**, **rostro no visible**— tienen bloque propio en cada prompt y los dos últimos están **declarados en positivo**. Deja **1 lección nueva** para el checklist Tipo A: **cuando la escena nueva contiene un elemento del mismo tipo que una pieza del producto** —acá, luces LED rojas de contraluz en el set y una LED roja en el spoiler del casco—, hay que **declarar la pieza como pieza y no como efecto de luz**, o el generador la absorbe dentro de la iluminación de la escena y la pierde; es distinta de la lección de la Simulación 26 (allá las dos piezas son del objeto y el problema es de **selección** en una edición; acá el gemelo es de la escena y el problema es de **absorción** en un traslado). Confirma además, como **segunda aparición**, la lección de la Simulación 34 sobre los reflejos de la escena nueva, con el matiz de que también aplica a **superficies negras pulidas** —visor opaco, piezas brillantes— y no solo a espejos evidentes. · agregada la sección de la **Simulación 36** (casco modular / flip up negro liso, 4 vistas: quitar stickers + visor 100% opaco negro + iluminación de set): **4 prompts** de edición puntual, uno por vista (frontal, lateral, trasera, superior), los cuatro 🔴 bloqueados hasta que el usuario confirme las 3 asunciones declaradas —el **logo "EDGE®" de la calota se conserva** y solo se quita el del film del visor, el **acabado real es negro mate** (la trasera se ve más brillante por la luz, no por la pintura) y **reiluminar entra dentro de "que el casco no cambie nada"**—; el total pasa de 83 a **87** prompts indexados. Es el **primer caso del repo donde se pide volver OPACO algo transparente**, la operación inversa de las Simulaciones 31 y 32, y el **primer caso donde se pide explícitamente que un conjunto de vistas se lea como un mismo set fotográfico**. Deja **2 lecciones nuevas** para el checklist Tipo A: (a) volver opaco algo transparente comparte la trampa de la operación inversa —la pieza tiene que seguir existiendo con su forma, su borde, su curvatura, su espesor y sus reflejos; lo que cambia es **qué deja pasar, no si está**—, con dos refuerzos propios de esta dirección (que la pieza opacada no se funda con una vecina del mismo color, y el **orden obligatorio limpiar→opacar** cuando la limpieza y la opacidad caen sobre la misma superficie, o el generador tapa los stickers con el negro en vez de borrarlos); y (b) cuando se piden varias vistas como un **set**, el bloque de iluminación tiene que estar redactado **con las mismas palabras** en todos los prompts, porque la coherencia del conjunto no emerge de optimizar cada vista por separado, con su corolario de auditoría (hay un chequeo que solo se puede hacer con las N imágenes puestas en tira). Se aplica además la regla de negocio de **no incluir stickers de certificación DOT/FMVSS** (Simulación 30). · agregada la sección de la **Simulación 33** (Boston, ficha vs. excel, Tipo C): **11 MATCH / 2 MISMATCH / 0 SIN DATO**, con la ficha **identificada por huella digital** como del Boston —es la única de las 8 columnas de su hoja donde "Diseño modular" y "Con luz LED" dan MATCH a la vez, y el **único modelo de todo el catálogo con LED confirmado**, corroborado además por las luces LED del panel lateral de la foto real de la Simulación 21—. **2 prompts nuevos** ✅ listos (A: 7 ítems, sale "liberación rápida del visor" y entra "Interior EPS"; B: 6 celdas, sale "canal para lentes" y entra "Material exterior ABS", conservando "Diseño modular" y "Con luz LED" que acá son reales, y sin tache); el total pasa de 81 a **83** prompts indexados. Deja dos cosas de nivel catálogo: el **hallazgo estructural de las dos hojas** (la hoja **EDGE** tiene una fila **PRECIO** de más y le faltan **KIT DE MECANISMO VISOR** y **CON MALETÍN DE LUJO**, con certificación **por modelo** y sin "22.06"), o sea que **la misma plantilla de ficha se aplica a dos líneas de producto con sets de specs distintos** —lo que explica por qué los mismos claims fallan modelo tras modelo—; y el **cierre del caso del tache de "hebilla micrométrica"** (Stellar falso / Shift correcto por casualidad / Boston falso → viene de la plantilla, hay que revisarlo modelo por modelo en las dos hojas). Se suman 4 pendientes nuevos (#35 a #38) y la 2ª aparición confirmada de **"HOMOLOGACÓN"**, ahora en la hoja EDGE. · **Shift (Simulación 15) actualizado tras la revisión con transcripción nítida**: el veredicto Tipo C **se mantiene en 9 MATCH / 4 MISMATCH / 0 SIN DATO**, pero la **prueba de huella digital** contra las 6 columnas EDGEPRO transcritas muestra que **la pieza no es la ficha del Shift ni de ningún modelo** (9/13 contra Shift, 10/13 contra Stellar, y los mismos 13 claims ya atribuidos a Hero, Kratos, Stellar y Vortex): es la **plantilla genérica publicada sin instanciar**. Se cerró el pendiente #24 —el **tache del ícono de hebilla micrométrica es correcto para Shift** (N/A) y falso para Stellar (X), **siendo el mismo arte**, o sea que acierta por casualidad—, se actualizó el #23 con el recuento de la certificación (6ª aparición de "DOT / FNVSS 510") y se agregó un **pendiente de decisión de negocio**: si conviene gastar 1 de las 6 celdas del grid en una feature ausente o pasarla a "HEBILLA DOBLE D" (recomendado, ya aplicado en el Prompt B, sin confirmar). Los 2 prompts del Shift se reemplazaron por sus versiones vigentes —**A con 7 ítems, B con 6 celdas**, manteniendo el tamaño original de las 2 piezas, con lienzo en números, geometría de separadores, conteo forzado, prohibición de adornos, los 3 bloques descritos por separado con "HOMOLOGACIÓN" bien escrito y la regla de taches matizada— y los 2 anteriores quedaron marcados ⛔ superados. · agregada la sección de la **Simulación 34** (casco integral negro mate con visor dorado levantado, foto lifestyle en una cafetería): **2 prompts** —(A) casco apoyado sobre la mesa, principal, y (B) hombre con el casco puesto, variante—, los dos marcados 🔴 bloqueados hasta que el usuario confirme cuál de las dos lecturas de "sentado con este casco" es la correcta; el total pasa de 73 a **75** prompts indexados. Es el primer caso de **traslado de escena** con superficie espejada, y deja una lección nueva para el checklist Tipo A: las superficies reflectantes de un objeto trasladado tienen que reflejar la escena nueva y no la del estudio de origen. · agregada la sección de la **Simulación 31** (casco jet con arte grafiti tipo Bob Esponja: edición puntual de 2 cambios — visor ahumado → transparente y quitar la guía de maquetación punteada), con su prompt marcado 🔴 bloqueado hasta confirmar con el usuario qué es "el palo"; el total pasa de 72 a 73 prompts indexados. · **Stellar (Simulación 14) actualizado tras la revisión con captura nítida**: el veredicto Tipo C pasó de 10/13 a **12 MATCH / 0 MISMATCH / 0 SIN DATO** porque la pieza auditada en la primera pasada no era la ficha vigente; se reemplazaron los Prompts A y B por sus versiones definitivas (A de reproducción fiel; B sin "doble visera", sin taches y con "Material exterior ABS alta resistencia" en la 6ª celda), se marcaron los 2 anteriores como ⛔ SUPERADOS, se cerró el pendiente de certificación (#19), se agregaron 3 pendientes nuevos (tache de arte, confirmación del ítem de reemplazo, subida de archivos) y se registró en un bloque aparte la **decisión de negocio de no comunicar "doble visera"** para que una auditoría posterior no la devuelva creyendo que es un olvido.
