...
### Intento 5 — Colorway 2 vista lateral, resultado auditado (sobrecorrección: el péndulo se fue para el otro lado)

**Estado:** ⚠️ Objetivo principal cumplido, con **sobrecorrección**. El defecto grande del Intento 4 (la masa roja continua) desapareció, pero la corrección se aplicó de más: el casco quedó prácticamente **monocromo negro/gris**. El veredicto del usuario: *"le bajó / eliminó el color rojo de casi todos lados, tenía que eliminar solo de la parte específica que habíamos quedado"*.

**El hallazgo más importante de este intento no es un defecto puntual: es un PÉNDULO.** Dos corridas consecutivas del mismo elemento, con la misma zona en juego, fallaron en direcciones opuestas:

| Intento | Superficie roja | Cómo se leía el casco | Estado de la flecha roja fina |
|---|---|---|---|
| **4** | Excesiva — calota superior + pico fundidos en una masa roja continua que ocupaba el tercio superior | Rojo como color principal, figura y fondo invertidos | ❌ Ausente — disuelta dentro de la masa roja (sin contraste posible) |
| **5** | Insuficiente — el rojo sobrevive solo en el pico/spoiler, un detalle chico en la parte alta delantera, la correa y dos o tres trazos sueltos cerca del pivote | Casco casi monocromo negro/gris; la zona roja de la calota alta-delantera desapareció | ❌ Ausente — el fondo ahora SÍ es oscuro, o sea que la condición de contraste se cumplió, y aun así la flecha no está |
| **Objetivo** | Intermedia — zona roja acotada en la parte alta y delantera de la calota (frente + arranque superior), que transiciona a oscuro hacia atrás y hacia abajo; pico rojo sólido; acentos finos | Fondo oscuro dominante con el rojo como acento **presente y reconocible** | ✅ UNA sola flecha/chevron roja de trazo delgado entre los chevrons grises concéntricos |

**Qué salió bien — y es mucho, no hay que arriesgarlo:** el resultado sostuvo absolutamente toda la base que este colorway venía construyendo intento tras intento.

- **Realismo fotográfico** — sigue siendo una fotografía de producto sobre fondo blanco de estudio: iluminación de estudio, sombras suaves, reflejos especulares sobre las superficies curvas y microtextura de pintura mate. Cero caída hacia la ilustración vectorial plana.
- **Forma y geometría del molde real** — silueta de la calota, pico frontal/spoiler, mentonera y ventilaciones intactas.
- **Visor tintado/oscuro** — mismo material y mismo tono.
- **Mecanismo de pivote del visor** — presente, con su textura real de fibra de carbono.
- **Correa roja** — en su lugar, con su trama de tela.
- **Goma negra del borde inferior** — cumple la regla de negocio de la sección 9 de `../orquestacion-agentes-paralelos.md`.
- **Wordmark "EDGE"** — presente, en su posición y con su tipografía.
- **Chevrones "XXX" de la mentonera** — presentes.
- **Ángulo, encuadre y fondo blanco** — sin drift.
- **Y el objetivo declarado del Intento 5 se cumplió:** la masa roja continua desapareció y **el cuerpo central del lateral quedó oscuro**, que era exactamente lo que se pedía.

**Qué falló (2 defectos):**

| # | Defecto | Qué se ve en el resultado | Qué debería verse (referencia de diseño) | Causa raíz |
|---|---|---|---|---|
| 1 | **Sobrecorrección — el rojo se fue de casi todos lados** | El casco quedó prácticamente **monocromo negro/gris**. El rojo sobrevive solo en el **pico frontal/spoiler**, un **detalle chico en la parte alta delantera**, la **correa** y **dos o tres trazos muy finos sueltos** cerca del pivote. **La calota superior delantera perdió el rojo que sí tenía que conservar.** | La calota tiene una **zona ROJA acotada en la parte ALTA Y DELANTERA** —la frente y el arranque superior de la calota— que después **transiciona a negro / gris muy oscuro hacia atrás y hacia abajo**. No es la calota entera, pero **tampoco es nada**: es una zona roja claramente presente y reconocible. | **La corrección estaba formulada casi enteramente en NEGATIVO, como sustracción.** El prompt del Intento 5 decía *"devolver el cuerpo central a negro"*, *"prohibido expandir el rojo"*, *"el rojo ocupa mucho menos superficie"*, *"si el rojo se lee como color principal, está mal"* — todo instrucciones de **cuánto sacar**. La única mención de dónde el rojo **sí** va era vaga: *"una FRANJA ACOTADA de la calota superior — acotada, no la calota entera"*, sin decir **dónde empieza**, **dónde termina** ni **cuánto ocupa**. Una instrucción que solo dice cuánto restar **no tiene piso**: el generador la aplica hasta el fondo, y "acotada" —una palabra puramente relativa, sin anclaje anatómico ni numérico— se resuelve como **"nada"**. |
| 2 | **La flecha roja delgada sigue sin aparecer — segundo intento consecutivo que falla** | Los chevrons concéntricos de esa zona salieron **todos en gris**, sin ninguno rojo entre ellos. | Sobre el fondo oscuro, grupos de **chevrons (flechas en "V" acostada) anidados y concéntricos**, la mayoría de trazo fino gris oscuro, y entre ellos **UNA SOLA flecha/chevron ROJA, también de trazo delgado**, que resalta por contraste. | **La causa raíz del Intento 4 quedó descartada empíricamente y hay que reemplazarla.** Allá el diagnóstico fue "un trazo rojo fino sobre fondo rojo es invisible"; el Intento 5 **cumplió esa condición** —el fondo de esa zona ahora sí es oscuro— y la flecha **igual no está**. O sea: el contraste era condición necesaria pero no suficiente. La causa real es otra: **un elemento único dentro de un grupo repetido se uniformiza con el grupo.** La flecha es *un* trazo rojo entre *muchos* chevrons grises **idénticos en forma**; al dibujar el grupo, el generador lo trata como un **patrón homogéneo** y le aplica un solo color a todos. Y el prompt del Intento 5, pese a describirla en positivo por grosor y por fondo, seguía tratándola como un **matiz de color dentro de un conjunto** (*"entre ellos hay UNA SOLA flecha roja"*) y gastaba cuatro líneas en **prohibiciones** (*prohibido engrosarla, duplicarla, convertirla en masa, apoyarla sobre fondo rojo*). Prohibir que un elemento salga mal no ayuda a que **exista**: ninguna de esas cuatro prohibiciones se viola cuando la flecha simplemente no se dibuja. |

**Diagnóstico general del intento — el patrón de proceso, que es lo más valioso de esta corrida:**

**Una corrección formulada solo como sustracción no tiene piso y produce el defecto inverso.** El Intento 4 falló por exceso de rojo; el Intento 5 corrigió describiendo exclusivamente **cuánto rojo sacar** y falló por defecto. Los dos fallos son la misma omisión vista desde los dos lados: **nunca se declaró el estado objetivo en positivo**. El prompt del Intento 4 no declaraba un **techo** de superficie roja y el generador la expandió hasta la masa continua; el prompt del Intento 5 declaró el techo pero no declaró un **piso** —ni ubicación anatómica, ni extensión, ni proporción— y el generador la contrajo hasta cero. Un prompt correctivo que solo dice "menos de esto" es tan incompleto como uno que solo dice "más de aquello": hay que **acotar en las dos direcciones**, y sobre todo hay que decir **qué tiene que haber**, no solo qué no tiene que haber. Se registra como ítem nuevo del checklist Tipo A en `../orquestacion-agentes-paralelos.md`.

**Corolario del segundo defecto, que es una causa raíz distinta y nueva:** **un elemento único dentro de un grupo de elementos repetidos tiende a uniformarse con el grupo.** El ítem ya registrado del checklist —"un detalle fino solo sobrevive si el prompt declara contra qué fondo tiene que contrastar"— era correcto pero incompleto: acá el contraste se consiguió y el elemento igual no apareció. Lo que falta es describirlo por su **posición ordinal dentro del grupo** (cuál de los chevrons anidados es el rojo, contando desde afuera o desde adentro), de modo que la instrucción deje de ser un **matiz de color** y pase a ser una **instrucción de dibujo con conteo forzado** — el mismo mecanismo que el checklist ya usa para los agregados ("EXACTAMENTE TRES, ni dos ni cuatro, contalas antes de entregar"). También se registra como ítem nuevo del checklist Tipo A.

**Qué hay que hacer:** corregirlo por **edición puntual sobre este resultado** (Intento 6, abajo), **no por regeneración**. Todo lo que salió bien —fotorrealismo, forma, visor, pivote de fibra de carbono, correa, goma negra, wordmark, chevrones "XXX", ángulo, encuadre y fondo— costó cinco intentos y no se pone en juego para reponer una zona de color y una flecha.

### Intento 6 — Colorway 2 vista lateral, edición puntual: reponer el rojo de la calota alta-delantera y HACER EXISTIR la flecha roja

**Enfoque:** edición puntual sobre el resultado del Intento 5 (se adjunta SOLO esa imagen), aplicando el ítem del checklist Tipo A *"Ajuste puntual sobre un resultado ya aprobado = edición, no regeneración completa"*. La base fotográfica y toda la geometría están bien y no se tocan.

**Los 3 cambios de enfoque respecto del prompt anterior:**
1. **El bloque de color se escribe en POSITIVO y con anatomía**: dónde nace la zona roja, hasta dónde llega, contra qué muere y qué proporción aproximada del casco ocupa. Se elimina la palabra "acotada" a secas, que fue exactamente lo que se resolvió como "nada".
2. **La flecha roja pasa de ser un matiz de color a ser una instrucción de dibujo con posición ordinal y conteo forzado**: se identifica *cuál* de los chevrons anidados es el rojo contando desde el borde exterior del grupo, y se exige contar los rojos antes de entregar (tienen que ser exactamente 1, y cero es un error tan grave como dos).
3. **Los límites se declaran en las DOS direcciones**, con los dos defectos históricos nombrados por su nombre, para que el péndulo no vuelva a irse para ningún lado.

<details><summary>Prompt usado (edición puntual: reponer color + hacer existir la flecha)</summary>
...
una fotografía de producto YA LOGRADA: su forma, su realismo, su
perspectiva, su iluminación y todo su dibujo están bien tal cual
están, y la mayor parte del trabajo ya está hecha. Se devuelve la
MISMA fotografía, con la MISMA geometría y el MISMO dibujo, con dos
correcciones puntuales de color y nada más.

CONTEXTO — POR QUÉ ESTA EDICIÓN (leelo antes de dibujar):
La imagen adjunta viene de corregir un exceso de rojo, y la
corrección se pasó de largo: se quitó rojo de lugares donde el rojo
SÍ tenía que quedarse. Hoy el casco se lee casi MONOCROMO NEGRO /
GRIS. Esta edición NO es "sacar más rojo": es exactamente lo
contrario — es DEVOLVER el rojo a las dos zonas específicas donde
el diseño lo tiene, dejando todo lo demás oscuro como está ahora.
...
- Todo el DIBUJO del livery: el rombo / marco geométrico angular
  concéntrico con sus líneas, el patrón de malla triangular con su
  densidad y su extensión, el texto chico dentro del marco, los
  grupos de chevrons concéntricos con su misma cantidad de trazos,
  las líneas grises finas y cada trazo en su misma posición exacta.
- EL CUERPO CENTRAL DEL LATERAL SIGUE SIENDO OSCURO. Eso ya está
  bien resuelto en la imagen adjunta y NO se revierte.

ESTADO OBJETIVO EN POSITIVO — DÓNDE VA EL ROJO, CON UBICACIÓN
ANATÓMICA Y PROPORCIÓN (este bloque describe lo que TIENE QUE
HABER, no lo que hay que sacar):
Cuando la edición esté terminada, el rojo del casco vive en estos
CUATRO lugares, y en ninguno más:

(A) ZONA ALTA Y DELANTERA DE LA CALOTA — ES LA QUE HOY FALTA Y HAY
    QUE REPONER. Es una zona de pintura roja continua que:
    - NACE en el arranque superior de la calota, justo detrás del
      pico frontal / spoiler, y cubre la FRENTE del casco (la parte
      alta y delantera, por encima de la abertura del visor);
    - se extiende hacia atrás sobre la parte alta de la calota
      cubriendo aproximadamente el TERCIO DELANTERO de la calota
      superior, y MUERE ANTES DE LLEGAR A LA MITAD del casco: la
      parte alta trasera de la calota queda oscura;
    - TRANSICIONA a negro / gris muy oscuro hacia ATRÁS y hacia
      ABAJO, con un borde de transición definido y limpio (no un
      degradé difuso que la desdibuje, y tampoco un corte que
      parezca otra pieza física: es pintura sobre una superficie
      continua);
    - hacia ABAJO no baja al cuerpo central del lateral: el rombo,
      el marco geométrico, los grupos de chevrons y la zona del
      wordmark "EDGE" siguen sobre fondo OSCURO.
(B) EL PICO FRONTAL / SPOILER, que es rojo sólido brillante, con su
    borde limpio contra la zona oscura. Ya está bien en la imagen
    adjunta: se conserva tal cual.
(C) LÍNEAS, CUÑAS Y CHEVRONS FINOS de acento, de trazo delgado,
    apoyados sobre el fondo oscuro del cuerpo central — incluida
    la flecha roja del bloque siguiente.
(D) LA CORREA, que ya es roja y queda como está.

PROPORCIÓN OBJETIVO: sumando (A) + (B) + (C) + (D), el rojo ocupa
aproximadamente entre un QUINTO y un CUARTO de la superficie
visible del casco en esta vista. Es un ACENTO CLARAMENTE PRESENTE
Y RECONOCIBLE, no el color dominante — pero tampoco un detalle
residual que haya que buscar. Un espectador que mire la foto tiene
que poder decir sin dudar "este casco es negro CON ROJO".

LÍMITE POR ARRIBA Y LÍMITE POR ABAJO — LOS DOS DEFECTOS YA
OCURRIERON, NINGUNO SE PUEDE REPETIR:
- DEMASIADO ROJO (defecto de dos intentos atrás, NO lo repitas):
  la calota superior entera y el pico fundidos en UNA SOLA MASA
  ROJA CONTINUA que ocupaba el tercio superior del casco, con el
  cuerpo central del lateral rojo y las líneas oscuras dibujadas
  encima — figura y fondo invertidos. PROHIBIDO. El rojo de la
  zona (A) no llega al cuerpo central del lateral, no cubre la
  calota entera y no se funde con el pico formando una sola masa
  que domine la vista.
- DEMASIADO POCO ROJO (defecto de la imagen adjunta, NO lo
  repitas): el casco casi MONOCROMO NEGRO / GRIS, con el rojo
  reducido al pico, la correa y dos o tres trazos sueltos, y la
  ZONA ALTA Y DELANTERA DE LA CALOTA SIN NADA DE ROJO. PROHIBIDO
  también. La zona (A) tiene que EXISTIR y verse a simple vista.
- El objetivo está EN EL MEDIO de esos dos extremos. No es "menos
  rojo que ahora" ni "más rojo que ahora" en general: es rojo
  EXACTAMENTE en las cuatro zonas listadas arriba, con la (A)
  repuesta y el resto como está.

CRÍTICO — LA FLECHA ROJA DELGADA TIENE QUE EXISTIR (ES EL SEGUNDO
INTENTO SEGUIDO QUE FALLA EN ESTO — ESTA VEZ TIENE QUE ESTAR SÍ O
SÍ):
En el cuerpo central del lateral, sobre fondo oscuro, hay uno o
más grupos de CHEVRONS: flechas en "V" acostada, apuntando hacia
adelante, ANIDADAS Y CONCÉNTRICAS una dentro de otra, todas del
mismo grosor de trazo fino. Hoy, en la imagen adjunta, TODOS los
chevrons de ese grupo son GRISES. Está mal: uno de ellos tiene que
ser ROJO.
Esto NO es un ajuste de matiz ni una excepción de una regla de
color: es una INSTRUCCIÓN DE DIBUJO CONCRETA. Ejecutala en dos
pasos, en este orden:

  PASO 1 — IDENTIFICAR: localizá el grupo de chevrons anidados
  concéntricos más grande y más visible del cuerpo central del
  lateral. Contá sus trazos desde el borde EXTERIOR del grupo
  hacia ADENTRO: el primero es el más externo (el más grande), el
  segundo es el que le sigue hacia adentro, y así.

  PASO 2 — PINTAR UNO SOLO: al SEGUNDO chevron contando desde
  afuera hacia adentro, cambiale el color de GRIS a ROJO. Ese
  trazo, y solamente ese, queda rojo. Todos los demás chevrons del
  grupo se quedan GRISES, exactamente como están hoy. (Si ese
  grupo tuviera menos de tres trazos, pintá el del medio; la regla
  es que el rojo NO es ni el más externo ni el más interno, sino
  uno intermedio, para que quede rodeado de grises por los dos
  lados.)

CONTEO FORZADO — ANTES DE ENTREGAR, CONTÁ LOS CHEVRONS ROJOS DE
ESE GRUPO. TIENE QUE HABER EXACTAMENTE UNO. CERO ES UN ERROR TAN
GRAVE COMO DOS. Si contás cero, la edición está MAL y hay que
rehacerla.

Cómo tiene que verse esa flecha roja:
- MISMO GROSOR DE TRAZO que los chevrons grises vecinos: es un
  trazo DELGADO, ni más fino ni más grueso que ellos. Lo único que
  la diferencia de sus vecinos es el COLOR.
- MISMA FORMA, MISMO LARGO y MISMA POSICIÓN que tenía cuando era
  gris: no se mueve, no se agranda, no se redibuja. Se le cambia
  el color y nada más.
- SOBRE FONDO OSCURO: el fondo del sector donde vive ese grupo
  queda negro / gris muy oscuro, para que el rojo del trazo
  contraste y se lea. Nunca sobre fondo rojo.
- PROHIBIDO ENGROSARLA, PROHIBIDO DUPLICARLA, PROHIBIDO
  CONVERTIRLA EN UNA MASA, EN UN PANEL, EN UNA CUÑA GRUESA O EN
  UNA SUPERFICIE RELLENA. Pero atención: la falla real de los dos
  intentos anteriores NO fue ninguna de esas — fue que la flecha
  NO APARECIÓ. La prioridad número uno es que EXISTA.
- Sigue siendo PINTURA sobre la superficie física del casco:
  acompaña la curvatura, se deforma en perspectiva y recibe la
  misma luz y las mismas sombras que el resto. Nunca una calcomanía
  plana ni un relieve.
...
Que una zona esté en la lista de cambios NO es permiso para
rehacerla. En las zonas que se tocan, lo ÚNICO que cambia es el
COLOR: qué superficie de la calota alta-delantera va en rojo y qué
trazo del grupo de chevrons va en rojo. TODO el resto del dibujo de
esas zonas se conserva IDÉNTICO — las líneas concéntricas del marco
con su misma cantidad y su mismo grosor, el patrón de malla
triangular con su misma densidad y su misma extensión, el texto
chico en su misma posición y con su mismo tamaño, los grupos de
chevrons con la misma cantidad de trazos, y cada trazo donde
estaba. No se rediseña, no se simplifica, no se reinterpreta: se
recolorea. Lo que hoy está dibujado sobre la calota alta-delantera
(líneas, malla, cualquier detalle) SIGUE AHÍ encima del rojo
repuesto, en la misma posición y con el mismo grosor.

NADA MÁS CAMBIA — PROHIBIDO ABSOLUTO:
- No redibujes el casco ni ninguna de sus partes.
- No cambies la forma, la geometría, la silueta ni las proporciones
  del molde real.
- No toques el visor tintado, el mecanismo de pivote con su fibra
  de carbono, la correa roja, la goma negra del borde inferior, el
  wordmark "EDGE" ni los chevrones "XXX" de la mentonera.
- No toques el pico frontal / spoiler: conserva su rojo sólido
  brillante, su forma, su acabado y su borde limpio.
- No devuelvas el cuerpo central del lateral al rojo: ese sector
  ya está bien en oscuro y se queda oscuro.
- No cambies el ángulo de cámara, el encuadre ni el fondo blanco de
  estudio.
- No vuelvas a renderizar el realismo fotográfico: la iluminación,
  las sombras, los reflejos y la microtextura de pintura mate de la
  imagen adjunta se conservan tal cual.
...
- No agregues ninguna pieza física, panel, aleta ni relieve que no
  ...
  plana sobre la superficie.
- SIN sticker "DOT" y SIN ninguna otra marca de certificación u
  homologación: regla de negocio del caso.
- No conviertas la imagen en una ilustración, un dibujo vectorial,
  un render plano ni un mockup: SIGUE SIENDO LA MISMA FOTOGRAFÍA DE
  PRODUCTO.

VERIFICACIÓN FINAL ANTES DE ENTREGAR — CINCO CHEQUEOS, LOS CINCO
OBLIGATORIOS:
1) ROJO REPUESTO: ¿la ZONA ALTA Y DELANTERA de la calota —la frente
   y el arranque superior, detrás del pico— tiene ahora pintura
   ROJA claramente visible, que transiciona a oscuro hacia atrás y
   hacia abajo? Si esa zona sigue negra o gris, la edición está MAL
   y NO cumpliste el pedido principal.
2) FLECHA ROJA: contá los chevrons ROJOS del grupo de chevrons
   anidados concéntricos del cuerpo central. ¿Cuántos hay? Tiene
   que haber EXACTAMENTE UNO, de trazo delgado, del mismo grosor
   que sus vecinos grises, sobre fondo oscuro. Si hay CERO, la
   edición está MAL: es el segundo intento seguido que falla en
   esto y no se puede entregar así. Si hay DOS o más, también está
   MAL.
3) NI DEMASIADO NI DEMASIADO POCO: ¿el rojo se lee como un acento
   PRESENTE Y RECONOCIBLE, ocupando alrededor de un quinto o un
   cuarto de la superficie visible? ¿NO volvió a aparecer la masa
   roja continua sobre el tercio superior del casco, y el cuerpo
   central del lateral sigue OSCURO? ¿Y el casco dejó de leerse
   como monocromo negro/gris?
4) DIBUJO INTACTO: ¿el marco geométrico concéntrico conserva todas
   sus líneas, el patrón de malla triangular su densidad y su
   extensión, el texto chico su posición y su tamaño, y cada grupo
   de chevrons su misma cantidad de trazos?
5) NADA MÁS CAMBIÓ: compará contra la imagen adjunta y confirmá que
   fuera de esas 2 zonas no cambió absolutamente nada — ni la
   forma, ni el visor, ni el pivote de fibra de carbono, ni la
   correa, ni la goma negra, ni el wordmark "EDGE", ni los
   chevrones "XXX", ni el pico frontal, ni el ángulo, ni el
   encuadre, ni el fondo, ni la iluminación, ni las sombras, ni los
   reflejos, ni la microtextura mate.
Si cualquiera de los cinco chequeos falla, corregilo antes de
entregar.
```
...
**Qué hay que hacer:** correr el prompt como edición de la imagen del Intento 5 (adjuntando SOLO esa imagen) y mandar el resultado para auditoría — verificar los 5 chequeos, con foco en los dos que vienen fallando: **(1)** que la zona alta y delantera de la calota tenga rojo visible transicionando a oscuro hacia atrás y hacia abajo, y **(2)** que haya **exactamente una** flecha/chevron roja delgada entre los chevrons grises concéntricos. Si la flecha vuelve a salir cero por tercera vez consecutiva, el próximo paso ya no es reforzar el texto: hay que **aislar el pedido** en una edición de un solo cambio (solo la flecha, sin ningún otro cambio de color en el prompt) para descartar que compita por atención con el bloque de proporción.
...
- **Colorway 2 lateral** — Intento 1 ✅ **aprobada** (3 puntos a verificar con zoom) → Intento 2 ❌ (la edición puntual terminó siendo un rediseño: el generador redibujó y simplificó toda la zona alta del lateral en vez de recolorearla, y encima el color no cambió) → Intento 3 ✅ **casi perfecto** (el recoloreo puro funcionó: el generador **no redibujó** la zona, todo el dibujo se conservó —marco concéntrico, malla con su densidad, texto chico, capas de líneas— y el rojo del pico frontal subió bien; el usuario dijo que quedó "increíble"), con **1 defecto**: el recoloreo quedó **incompleto** —un grupo de barras/franjas horizontales cortas en la parte baja-izquierda del sector siguió en rojo en vez de pasar a negro— → Intento 4 ⚠️ **bien en lo general, con 2 defectos de color** (el usuario: **"se excedió mucho el tono rojo"** y **"la flecha delgada y roja de la referencia del dibujo no sale bien"**): se sostuvieron realismo, forma, visor, pivote de fibra de carbono, correa roja, goma negra, wordmark "EDGE" y chevrones "XXX", pero la calota superior y el pico frontal quedaron como una **masa roja continua** que ocupa el tercio superior del casco —**figura y fondo invertidos** respecto del mockup, donde el cuerpo central del lateral es oscuro y el rojo es acento— y la **flecha roja delgada se perdió** dentro de esa masa por falta de contraste → **Intento 5 ⚠️ generado — SOBRECORRECCIÓN: el péndulo se fue para el otro lado** (el usuario: *"le bajó / eliminó el color rojo de casi todos lados, tenía que eliminar solo de la parte específica que habíamos quedado"*): el objetivo principal se cumplió —la masa roja continua desapareció y el cuerpo central del lateral quedó oscuro— y toda la base se sostuvo intacta (fotorrealismo, forma del molde, visor tintado, pivote de fibra de carbono, correa roja, goma negra, wordmark "EDGE", chevrones "XXX", ángulo, encuadre y fondo), pero el casco quedó **prácticamente monocromo negro/gris**: el rojo sobrevive solo en el pico/spoiler, un detalle chico en la parte alta delantera, la correa y dos o tres trazos sueltos, y **la calota superior delantera perdió el rojo que sí tenía que conservar**; y la **flecha roja delgada sigue sin aparecer por segundo intento consecutivo**, esta vez **con el fondo oscuro ya conseguido** —o sea que la causa raíz de contraste diagnosticada en el Intento 4 queda descartada y se reemplaza por otra: un elemento único dentro de un grupo repetido se uniformiza con el grupo → **Intento 6 🔴 pendiente de generar**, planteado como **edición puntual** sobre ese resultado, con el **estado objetivo declarado en positivo y con anatomía** (dónde nace la zona roja, hasta dónde llega, contra qué muere y qué proporción ocupa), la **flecha convertida en instrucción de dibujo con posición ordinal y conteo forzado** (el segundo chevron contando desde afuera hacia adentro; exactamente uno rojo, cero es tan grave como dos) y **límites declarados en las dos direcciones** para que el péndulo no vuelva a irse para ningún lado.
...
Octavo patrón, descubierto con el Intento 5 de la lateral del Colorway 2 y el más caro de todos porque cuesta **dos corridas** en vez de una: **una corrección formulada solo como SUSTRACCIÓN no tiene piso y produce el defecto inverso — el efecto péndulo.** El Intento 4 falló por **exceso** de rojo (masa roja continua sobre el tercio superior, figura y fondo invertidos); el Intento 5 corrigió describiendo exclusivamente **cuánto rojo sacar** —*"devolver el cuerpo central a negro"*, *"prohibido expandir el rojo"*, *"el rojo ocupa mucho menos superficie"*, *"si el rojo se lee como color principal, está mal"*— y falló por **defecto**: el casco salió casi monocromo negro/gris y la zona roja de la calota alta-delantera, que sí tenía que quedarse, desapareció. Los dos fallos son la **misma omisión vista desde los dos lados**: nunca se declaró el **estado objetivo en positivo**. Al Intento 4 le faltaba un **techo**; al Intento 5 le faltaba un **piso**. Y la única mención de dónde el rojo sí iba era *"una franja acotada de la calota superior — acotada, no la calota entera"*: sin decir **dónde empieza**, **dónde termina** ni **cuánto ocupa**, "acotada" —una palabra puramente relativa— se resuelve como **"nada"**. La regla es declarar **qué tiene que haber**, con **ubicación anatómica concreta** y **proporción aproximada**, y acotar en las **dos direcciones**, nombrando los dos defectos ya ocurridos. **Corolario, con causa raíz propia y distinta de la ya registrada: un elemento único dentro de un grupo de elementos repetidos tiende a UNIFORMARSE con el grupo.** La flecha roja falló **dos intentos seguidos**, y el segundo con la condición de contraste ya cumplida (fondo oscuro conseguido), lo que **descarta empíricamente** el diagnóstico anterior de "un trazo rojo fino sobre fondo rojo es invisible": el contraste era necesario pero no suficiente. La causa real es que la flecha es *un* trazo rojo entre *muchos* chevrons grises **idénticos en forma**, y el generador dibuja el grupo como un **patrón homogéneo** con un solo color. Prohibir (*prohibido engrosarla, duplicarla, convertirla en masa*) no ayuda a que **exista** — ninguna de esas prohibiciones se viola cuando el elemento simplemente no se dibuja. Hay que describirlo por su **posición ordinal dentro del grupo** (cuál de los chevrons anidados es el rojo, contando desde afuera hacia adentro) y cerrar con **conteo forzado** ("exactamente uno; cero es tan grave como dos"), convirtiendo el pedido de **matiz de color** en **instrucción de dibujo** — el mismo mecanismo que el checklist ya usa para los agregados.
